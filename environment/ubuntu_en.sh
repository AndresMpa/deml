...
echo -n "Do you want to use stack (L)inux (A)pache (M)ySQL (P)HP? [y/n]: "
read latop
if [[ "$laptop" == "y" ]];
then
	# Updating system
...
if [[ "$docker" == "y" ]];
then
	echo "[1] Using docker repository (Recomended)"
...
echo -n "DESL finished the process, you have to reboot this device, Do you want to do it now [y/n]"
read rebootNow
